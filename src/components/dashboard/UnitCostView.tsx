  ...
  return (
    <>
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={chartData}>
              ...
              <YAxis yAxisId="left" tick={{ fill: "#64748b", fontSize: 12 }} tickFormatter={(v) => `R$${v}`} />
              <YAxis yAxisId="right" orientation="right" tick={{ fill: "#64748b", fontSize: 12 }} tickFormatter={(v) => `${v}%`} />
              <Tooltip
              ...
              />
              <Bar yAxisId="left" dataKey="cogs" stackId="cost" fill="#64748b" radius={[0, 0, 0, 0]} />
              <Bar yAxisId="left" dataKey="fixos" stackId="cost" fill="#94a3b8" radius={[4, 4, 0, 0]} />
              <Line yAxisId="right" type="monotone" dataKey="margem" stroke="#3b82f6" strokeWidth={3} dot={{ fill: "#3b82f6", r: 4 }} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
        <div className="mt-3 text-xs text-slate-500 leading-relaxed">
          <span className="text-slate-400 font-medium">Leitura:</span> À medida que a base cresce, os custos fixos se diluem por cliente, melhorando a margem líquida.
        </div>
        <div className="mt-2 text-xs text-slate-500 leading-relaxed">
          <span className="text-slate-400 font-medium">Importante:</span> A margem líquida neste gráfico reflete a <strong className="text-slate-200">fase de crescimento</strong> (50 → {activeCount} clientes), onde os custos de marketing e aquisição são maiores. A margem no <strong className="text-slate-200">Investor View</strong> assume a base já estabilizada no target, com marketing apenas de reposição de churn — por isso é naturalmente mais alta.
        </div>
      </Section>
      ...
      <Section title={`Custo por cliente — separado por plano (${activeCount} clientes ativos)`}>
            ...
            <tbody>
              {rows.map((r, idx) => (
                <tr key={idx} className={`border-b border-slate-700/50 ${idx % 2 === 0 ? "bg-slate-800/30" : ""}`}>
                  <td className="py-3 px-3 text-slate-200 font-medium">{r.plano}</td>
                  <td className="py-3 px-3 text-right text-slate-200 font-medium">{brl(r.preco)}</td>
                  <td className="py-3 px-3 text-right text-slate-400">{brl(r.cogs)}</td>
                  <td className="py-3 px-3 text-right text-slate-300 font-medium">{pct(r.margemBruta)}</td>
                  <td className="py-3 px-3 text-right text-slate-400">{brl(r.fixedAlloc)}</td>
                  <td className="py-3 px-3 text-right text-slate-200 font-medium">{brl(r.unitCostTotal)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="mt-3 text-xs text-slate-500 leading-relaxed">
          <span className="text-slate-400 font-medium">Nota:</span> Rateio de custos fixos baseado em <strong className="text-slate-200">{activeCount}</strong> clientes ativos (mês 12 do cenário selecionado). COGS por plano é o custo variável por cliente/mês.
        </div>
      </Section>
    </>
  );
